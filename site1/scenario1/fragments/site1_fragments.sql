...
-- Règle : IDCATEG = 50 (toutes quantités)
-- BDD   : BDDVENTE
-- NB    : Tables suffixées _sc1 pour éviter conflit avec Scenario 2
--
-- CORRECTION : la règle de fragmentation est uniquement
-- basée sur la CATÉGORIE. La condition QUANTITE > 100 a été
-- supprimée car elle créait une fragmentation incomplète :
-- les enregistrements IDCATEG=50 AND QUANTITE<=100 n'étaient
-- assignés à aucun site.
-- ============================================================
...
-- -------------------------------------------------------
-- 3. LigneCommandes1_sc1 : toutes les lignes liées
--    aux produits de catégorie 50 (sans condition sur quantité)
-- -------------------------------------------------------
...
-- VERIFICATION
-- -------------------------------------------------------
-- SELECT COUNT(*) FROM Produits1_sc1;        -- 3 produits catégorie 50
-- SELECT COUNT(*) FROM LigneCommandes1_sc1;  -- 10 lignes (IDPRODUIT 1,2,3)
-- SELECT COUNT(*) FROM Commandes1_sc1;       -- commandes 1-5
-- SELECT COUNT(*) FROM Clients1_sc1;         -- clients 1,2,3
