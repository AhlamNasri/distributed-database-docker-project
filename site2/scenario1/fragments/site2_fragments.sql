...
-- Règle : IDCATEG = 35 AND QUANTITE <= 100
-- BDD   : BDDVENTE2
...
-- -------------------------------------------------------
-- 3. LigneCommandes2_sc1 : lignes liées aux produits Site2
--    RÈGLE : IDCATEG = 35 AND QUANTITE <= 100
-- -------------------------------------------------------
...
WHERE p.IDCATEG = 35 AND lc.QUANTITE <= 100;

ALTER TABLE LigneCommandes2_sc1 ADD PRIMARY KEY (IDLIGNECOMMANDE);
ALTER TABLE LigneCommandes2_sc1 ADD CONSTRAINT chk_sc1_site2_qte
    CHECK (QUANTITE <= 100);
...
-- VERIFICATION
-- -------------------------------------------------------
-- SELECT COUNT(*) FROM Produits2_sc1;
-- SELECT COUNT(*) FROM LigneCommandes2_sc1;
-- SELECT COUNT(*) FROM Commandes2_sc1;
-- SELECT COUNT(*) FROM Clients2_sc1;
