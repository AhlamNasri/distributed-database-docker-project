...
COLUMN CMD5_PRESENTE FORMAT 999999
COLUMN CMD6_PRESENTE FORMAT 999999
...
PROMPT === Donnees de base inserees pour Site2 Scenario 1 ===
...
-- -------------------------------------------------------
PROMPT Test INSERT invalide (produit hors categorie 35)
BEGIN
...
-- -------------------------------------------------------
PROMPT Test UPDATE invalide (mauvaise categorie)
BEGIN
...
-- -------------------------------------------------------
PROMPT Test DELETE valide (cascade commande)
BEGIN
    deleteligne2_sc1(403);  -- Dernière ligne de commande 6
END;
/

SELECT COUNT(*) AS NB_LIGNES FROM LigneCommandes2_sc1;
SELECT COUNT(*) AS CMD6_PRESENTE FROM Commandes2_sc1 WHERE IDCOMMANDE = 6;
...
SELECT COUNT(*) AS CMD5_PRESENTE FROM Commandes2_sc1 WHERE IDCOMMANDE = 5;

-- -------------------------------------------------------
-- 11. Test DELETE invalide (ligne inexistante)
-- -------------------------------------------------------
PROMPT Test DELETE invalide
BEGIN
...
-- -------------------------------------------------------
PROMPT Test TRIGGER INSERT bloque (categorie)
BEGIN
...
-- -------------------------------------------------------
PROMPT Test TRIGGER INSERT bloque (quantite)
BEGIN
    INSERT INTO LigneCommandes2_sc1 (IDLIGNECOMMANDE, IDCOMMANDE, IDPRODUIT, QUANTITE, REMISE)
...
-- 14. Résumé final
-- -------------------------------------------------------
PROMPT Resume final Site2 Scenario 1
SELECT IDLIGNECOMMANDE, IDCOMMANDE, IDPRODUIT, QUANTITE, REMISE
FROM LigneCommandes2_sc1 ORDER BY IDLIGNECOMMANDE;

SELECT COUNT(*) AS NB_LIGNES FROM LigneCommandes2_sc1;
SELECT COUNT(*) AS NB_COMMANDES FROM Commandes2_sc1;
SELECT COUNT(*) AS NB_LOGS FROM LOG_SITE2_SC1;
