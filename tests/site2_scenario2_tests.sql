...
COLUMN CMD4_PRESENTE FORMAT 999999
...
PROMPT === Donnees de base inserees pour Site2 Scenario 2 ===
...
-- -------------------------------------------------------
PROMPT Test INSERT invalide (Qte >= 100)
BEGIN
...
-- -------------------------------------------------------
PROMPT Test UPDATE invalide (Qte >= 100)
BEGIN
...
-- -------------------------------------------------------
PROMPT Test DELETE valide
BEGIN
    deleteligne2(303);  -- Dernière ligne de la commande 4 → commande doit être supprimée
END;
/

SELECT COUNT(*) AS NB_LIGNES FROM LigneCommandes2;
SELECT COUNT(*) AS CMD4_PRESENTE FROM Commandes2 WHERE IDCOMMANDE = 4;

-- -------------------------------------------------------
-- 7. Test TRIGGER INSERT bloqué
-- -------------------------------------------------------
PROMPT Test TRIGGER INSERT bloque
BEGIN
...
-- 8. Résumé final
-- -------------------------------------------------------
PROMPT Resume final Site2 Scenario 2
SELECT IDLIGNECOMMANDE, IDCOMMANDE, IDPRODUIT, QUANTITE, REMISE
FROM LigneCommandes2 ORDER BY IDLIGNECOMMANDE;

SELECT COUNT(*) AS NB_LIGNES FROM LigneCommandes2;
SELECT COUNT(*) AS NB_COMMANDES FROM Commandes2;
SELECT COUNT(*) AS NB_LOGS FROM LOG_SITE2;
